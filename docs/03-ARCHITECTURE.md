...
**Short Practice Quiz v1:** Default Quiz attempt length is 10 questions (`ACTIVITY_DEFAULTS.quiz.questionCount`). Scoring stays percentage-based (`correct / total`). A 10-question bank slice is near-balanced A/B/C/D (3/3/2/2). Historical events store `scorePercentage` only, so older 20-question attempts remain readable without migration. Millionaire stays 10 questions. Present Simple Flash Cards use a dedicated 12-card review deck, not the Quiz bank, and are not in `ACTIVITY_DEFAULTS`.

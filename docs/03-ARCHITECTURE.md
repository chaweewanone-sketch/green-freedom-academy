...
- Activity result screens persist first, then may show a primary CTA. Quiz Result classifies the **current** attempt via the shared score-band helper: weak `ทำ Quiz อีกครั้ง` and developing `ฝึก Quiz` remount the same in-memory session to the intro Start screen; strong `เล่น Millionaire`. Home / Journey / Resume still use historical `averageQuizScore`. Completed Millionaire → next lesson or `/dashboard`. Weak/developing Millionaire and Flash Cards keep `กลับหน้าหลักนักเรียน` · `เริ่มใหม่` · `กลับไปบทเรียน`. This is display guidance, not route locking.

...
**10-stage game v1:** The Millionaire activity keeps internal id `millionaire` and the same 10-question assessment session. Student-facing play is `เกมพิชิต 10 ด่าน`: a display-only ด่าน/star ladder, game-scoped `.gfaGame*` layout, text+color feedback, and an explicit continue (`ไปด่านต่อไป →` / `ดูผลเกม`) instead of 700ms auto-advance. Result copy shows stars for the correct count; Sprint 41 current-attempt CTAs are unchanged. Shared `.millionaire*` CSS used by Quiz is not mutated.

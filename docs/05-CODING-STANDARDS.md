...
- Lesson Entry comes from `buildLessonEntry(lessonSlug, events)` — compose curriculum status + this lesson’s `evaluateLessonJourney` for `/lesson/[slug]`. Do not lock routes. Do not show another lesson’s recommendation as if it belongs to the viewed lesson
...
- The journey engine, lesson evaluator, active-lesson resolver, recommendation engine, curriculum progress builder, resume builder, student-home view model, and lesson-entry view model must not import `localStorage` or `LearningHistoryRepository`

...
| Activity Result forward CTA | CURRENT | After persist, Quiz Result primary CTA follows the current attempt: weak `ทำ Quiz อีกครั้ง`, developing `ฝึก Quiz` (intro retry), strong `เล่น Millionaire`. Home / Journey / Resume still use historical Quiz average. Completed Millionaire → next lesson or dashboard. Weak/developing Millionaire and Flash Cards keep the generic Result buttons |

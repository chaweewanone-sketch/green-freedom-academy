...
| Activity Result forward CTA | CURRENT | After persist, Quiz Result shows the engine next action: weak `ทำ Quiz อีกครั้ง`, developing `ฝึก Quiz` (in-session restart), strong `เล่น Millionaire`. Completed Millionaire → next lesson or dashboard. Weak/developing Millionaire and Flash Cards keep the generic Result buttons |
